...
module.exports = {
  ...
  async getUserDisplay(user) {
    const displays = await knex('displays')
      .select('displays.type as display', 'display_sensor_items.html_id as name', 'display_sensor_items.sensor_id', 'displays.id', 'sensor_types.type as type', 'sensors.name as sensor')
      .where({ user_id: user })
      .join('display_sensor_items', 'displays.id', 'display_id')
      .join('sensors', 'sensors.id', 'display_sensor_items.sensor_id')
      .join('sensor_types', 'sensors.type_id', 'sensor_types.id')
      .groupBy('displays.id', 'display_sensor_items.id', 'sensor_types.type', 'sensors.name');

      console.log("getUserDisplay -> displays", displays);
    const result = {};
    displays.forEach(item => {
      if (item.display in result) {
        const display = result[item.display];
        display.push(item);
        // result[item.display] = display;
      } else {
